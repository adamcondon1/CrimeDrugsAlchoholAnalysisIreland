...
# Changing the Type of Offence into Abbreviations
Crime_Ireland$`Abbr. of Crime`[Crime_Ireland$`Abbr. of Crime` == "ATTEMPTS/THREATS TO MURDER/ASSAULTS/ HARASSMENTS AND RELATED OFFENCES"] <- "Attempts/TTM/A/HARO"
...
# Ensuring the changes were made successfully
str(Crime_Ireland)
...
plot(Crime_Ireland$`Abbr. of Crime`, 
     ...
     col = colors(14), 
     main = "Offences Type", 
     xlab = "Abb. of Crime Types", 
     ylab = "No. Of Crimes Recorded")
...
# WEB SCRAPING HTML parser Loading 2020 Gardai Offenses Not reported on 
library(rvest)
library(dplyr)

link = "https://data.gov.ie/dataset/cvs49-reason-for-not-reporting-some-or-all-incidents-to-an-garda-siochana?package_type=dataset"
page = read_html(link)

title = page %>% html_nodes("h1") %>% html_text()
where = page %>% html_nodes(".dataset-top-meta div:nth-child(1) strong a") %>% html_text()
description = page %>% html_nodes(".embedded-content p") %>% html_text()
rating = page %>% html_nodes(".star-rating-stars") %>% html_text()
csv = page %>% html_nodes(".col-sm-6:nth-child(1) .btn-download
") %>% html_text()


ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana = data.frame(title, where, description, rating, csv, stringsAsFactors = FALSE)
write.csv(ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana, ".csv")

#Checking for any missing values when extracted
library(mice)
library(VIM)
md.pattern(ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana)
missing_values <- aggr(ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana, prop = FALSE, numbers = TRUE)

library(mice)
library(VIM)
md.pattern(ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana)
missing_values <- aggr(ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana, prop = FALSE, numbers = TRUE)
#Checking to see if loaded correctly basic Histogram plot of values
hist(AirPassengers, 
     main="Histogram for Reasons", 
     xlab="Reasons", 
     ylab="No. Of Reasons",
     border="orange", 
     col="green",
...
#Loading ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana dataset
ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana <- read.csv("ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana.csv", header = TRUE)
...
#Merging ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana stats 2020 into main final crime dataset
FinalCrimeDataset <- merge(FinalCrimeDataset, ReasonForNotReportingSomeOrAllIncidentstoAnGardaSiochana, by.x = "Year", by.y  = "Year",
...
# Because we have merged another dataset we need to look at the missing value again
library(mice)
library(VIM)
...
# eigenvalues measure the amount of variation retained by each principal component. ref: https://cran.r-project.org/web/packages/matlib/vignettes/eigen-ex1.html
library("factoextra")
...
# This code shows the top 20 variables 
...
#The variables shown are continous so corr coefficient testing is carried out to examine the relationships between variables
